...
void Solver_MCSVM_CS::Solve(double *w)
{
	...
	for(i=0;i<l;i++)
	{
		...
		scalar_t *xi = prob->x[i].values;
		QD[i] = 0;
		for(int j=0;j<dim;++j)
			QD[i] += (double)(xi[j])*(double)(xi[j]);
#else
		feature_node *xi = prob->x[i];
		QD[i] = 0;
		while(xi->index != -1)
		{
			QD[i] += (double)(xi->value)*(double)(xi->value);
			xi++;
		}
#endif
...
void solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp, double Cn)
{
	...
	for(i=0; i<l; i++)
	{
		...
		while (xi->index != -1)
		{
			xTx[i] += (double)(xi->value)*(double)(xi->value);
	...
	double Cp, double Cn)
{
	...
	for(j=0; j<w_size; j++)
	{
		...
		for(int i=0;i<l;++i)
		{
			dim=prob_col->x[i].dim;
			if(j < dim)
			{
				x=prob_col->x[i].values;
				scalar_t val = x[j];
				x[j] *= y[i]; // x->value stores yi*xij
				xj_sq[j] += C[GETI(i)]*val*val;
			}
		}
#else
		x = prob_col->x[j];
		...
		for(s=0; s<active_size; s++)
		{
			...
			if(num_linesearch >= max_num_linesearch)
			{
				...
				for(int i=0; i<w_size; i++)
				{
					...
					for(int k=0;k<l;++k)
					{
						dim=prob_col->x[k].dim;
						if(i < dim)
						{
							x=prob_col->x[k].values;
							b[k] -= w[i]*x[i];
						}
					}
#else
	...
	for(j=0; j<w_size; j++)
	{
		...
		for(int i=0;i<l;++i)
		{
			...
			{
				x=prob_col->x[i].values;
				x[j] *= prob_col->y[i]; // restore x->value
			}
		}
#else
		x = prob_col->x[j];
	...
	for(j=0; j<w_size; j++)
	{
		...
		for(int i=0;i<l;++i)
		{
			...
			if(j < dim)
			{
				...
				C_sum[j] += C[GETI(i)];
				if(y[i] == -1)
					xjneg_sum[j] += C[GETI(i)]*val;
				else
					xjpos_sum[j] += C[GETI(i)]*val;
			}
		}
#else
		x = prob_col->x[j];
		...
		for(s=0; s<active_size; s++)
		{
			...
			for(int i=0;i<l;++i)
			{
				...
				if(j < dim)
				{
					...
					double tmp1 = x[j]/(1+exp_wTxind);
					double tmp2 = C[GETI(i)]*tmp1;
			...
			for(num_linesearch=0; num_linesearch < max_num_linesearch; num_linesearch++)
			{
				...
				for(int i=0;i<l;++i)
				{
					...
					{
						x=prob_col->x[i].values;
						scalar_t exp_dx = exp(d*x[j]);
						exp_wTx_new[i] = exp_wTx[i]*exp_dx;
						cond += C[GETI(i)]*log((1+exp_wTx_new[i])/(exp_dx+exp_wTx_new[i]));
					}
				}
#else
				int i = 0;
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn)
{
	...
	switch(param->solver_type)
	{
		...
		{
#ifdef _DENSE_REP
			solve_l1r_l2_svc(prob, w, eps*min(pos,neg)/prob->l, Cp, Cn);
#else
			problem prob_col;
		...
		{
#ifdef _DENSE_REP
			solve_l1r_lr(prob, w, eps*min(pos,neg)/prob->l, Cp, Cn);
#else
			problem prob_col;
			feature_node *x_space = NULL;
			transpose(prob, &x_space ,&prob_col);
...
int predict_values(const struct model *model_, const struct feature_node *x, double *dec_values)
{
	...
	{
		for(i=0;i<nr_w;i++)
			dec_values[i] += w[idx*nr_w+i]*lx[idx];
	}
#else

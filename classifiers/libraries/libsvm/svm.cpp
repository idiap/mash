...
double Kernel::dot(const svm_node *px, const svm_node *py)
{
	...
	//	sum += (px->values)[i] * (py->values)[i];
		sum += (double)(px->values)[i] * (double)(py->values)[i];
	return sum;
}
	...
	switch(param.kernel_type)
	{
		...
		case RBF:
		{
			...
			//	double d = x->values[i] - y->values[i];
				double d = (double)x->values[i] - (double)y->values[i];
				sum += d*d;
			}
			for (; i < x->dim; i++)
			//	sum += x->values[i] * x->values[i];
				sum += (double)x->values[i] * (double)x->values[i];
			for (; i < y->dim; i++)
			//	sum += y->values[i] * y->values[i];
				sum += (double)y->values[i] * (double)y->values[i];
#else

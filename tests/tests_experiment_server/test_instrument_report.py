...
def process(args):
    ...
    try:
        module = __import__(expected_file)
    ...
    # Checking of the data report
    write('Checking of the data report...')

    archive = tarfile.open('test_data_report.tar.gz')

    members = map(lambda x: x.name, archive.getmembers())

    expected_files = ['configuration.xml', 'predictor.data', 'mash/heuristic_statistics.data']
    if appserver_type == 'image':
        expected_files.extend(['labels.xml', 'images.xml'])
    
    for name in expected_files:
        if not(name in members):
            error("No '%s' file found in the data report" % name)

    report = archive.extractfile(report_file)
    if report is None:
        error("No '%s' file found in the data report" % report_file)

    content = report.read()

    offset = 0
    for count, values in expected:
        for i in range(0, count):
            for value in values:
                next = offset + len(value) + 1
                if content[offset:next] != value + '\n':
                    print
                    print content
                    print
                    error("Bad data content, expected '%s' at offset %d, got '%s'" % (value, offset, content[offset:next-1]))
                offset = next
...
if __name__ == "__main__":
    ...
    usage = "Usage: %prog {image | maze} APPSERVER_DIR EXPERIMENT_SERVER_DIR SETTINGS REPORT_FILE EXPECTED_FILE"

...
def process(args):
    ...
    expected_model_file     = args[5]
    
    if len(args) == 7:
        expected_internal_data_file = args[6]
    else:
        expected_internal_data_file = None
    ...
    for command in commands:
        sendCommand(client, Message.fromString(command), ['OK'])
    ...
    # Checking of the saved model
    write('Checking of the saved model...')

    archive = tarfile.open('test_data_report.tar.gz')

    model = archive.extractfile('predictor.model')
    if model is None:
        error("No model found in the data report")

    content = model.read()

    inFile = open('%s/%s' % (script_dir, expected_model_file), 'r')
    expected = inFile.read()
    inFile.close()

    if re.match(expected, content) is None:
        error("Bad model content, expected:\n----------\n%s\n----------\ngot:\n----------\n%s\n----------\n" % (expected, content))

    if expected_internal_data_file is not None:
        internal_data = archive.extractfile('predictor.internal')
        if internal_data is None:
            error("No predictor internal data found in the data report")

        content = internal_data.read()

        inFile = open('%s/%s' % (script_dir, expected_internal_data_file), 'r')
        expected = inFile.read()
        inFile.close()

        if content != expected:
            error("Bad predictor internal data content, expected:\n----------\n%s\n----------\ngot:\n----------\n%s\n----------\n" % (expected, content))
...
if __name__ == "__main__":
    ...
    usage = "Usage: %prog {image | maze} APPSERVER_DIR EXPERIMENT_SERVER_DIR SETTINGS USE_SANDBOXING EXPECTED_MODEL_FILE [EXPECTED_INTERNAL_DATA_FILE]"

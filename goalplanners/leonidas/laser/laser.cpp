...
bool laser::learn(ITask* task) {
    ...
    if (task->capabilities() & IAS_CAP_TRAJECTORIES)
        nbLaserRounds = std::min(nbLaserRounds, task->nbTrajectories());
    ...
    for (unsigned int LaserRound = 0; LaserRound<nbLaserRounds;LaserRound++) {
        ...
        while ((task->result()==RESULT_NONE) && (nbAct<max_nb_actions))
        {
            ...
            task->performAction(task->suggestedAction(), reward);
            nbAct++;
        }
    ...
    if (diff_labels.nblabels > 1)
        Laser_predictor->train(GPcis_train, train_error);

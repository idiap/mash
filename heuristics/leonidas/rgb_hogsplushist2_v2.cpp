...
//------------------------------------------------------------------------------
class rgb_hogsplushist: public Heuristic
{
    //_____ Construction / Destruction __________
public:
    rgb_hogsplushist();
    virtual ~rgb_hogsplushist();
...
rgb_hogsplushist::rgb_hogsplushist()
{
...
unsigned int rgb_hogsplushist::dim()
{
  ...
  return 3+(hist1-2-(block_size-1))*(hist2-2-(block_size-1))*or_bins*block_size*block_size;

}


void rgb_hogsplushist::prepareForCoordinates()
{
    ...
    scalar_t block_norm;
    scalar_t dx[3],dy[3],grad_or,grad_mag,temp_mag,rgbh[3];
    scalar_t Xc,Yc,Oc;
    ...
    rgbh[0]+= pixels[y0+y][x0+x].r;
    rgbh[1]+= pixels[y0+y][x0+x].g;
    rgbh[2]+= pixels[y0+y][x0+x].b;
...
if (rgbh[0]+rgbh[1]+rgbh[2] > 0){
dth_des[h_c] = rgbh[0]/(rgbh[0]+rgbh[1]+rgbh[2]);
dth_des[h_c+1] = rgbh[1]/(rgbh[0]+rgbh[1]+rgbh[2]);
dth_des[h_c+2] = rgbh[2]/(rgbh[0]+rgbh[1]+rgbh[2]);
}
else {
dth_des[h_c] = 1/3;
dth_des[h_c+1] = 1/3;
...
void rgb_hogsplushist::finishForCoordinates()
{
   delete[] dth_des;
}


scalar_t rgb_hogsplushist::computeFeature(unsigned int feature_index)
{

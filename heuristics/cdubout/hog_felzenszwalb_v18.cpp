...
private:
	...
	static float ATAN2_TABLE[512][512];
};

float HoGHeuristic::ATAN2_TABLE[512][512] = {{0.0f}};
...
void HoGHeuristic::prepareForImage()
{
	...
	if (ATAN2_TABLE[0][0] == 0.0f) {
		for (int dy = -255; dy <= 255; ++dy) {
			for (int dx = -255; dx <= 255; ++dx) {
				...
				angle = max(angle * float(9.0 / M_PI) + 9.0f, 0.0f);
	...
	// Upscale the gradients of the last row and last column
	if (width & 7) {
		const float scale = 8.0f / float(width & 7);

		for (int i = 0; i < height_; ++i)
			for (int j = 0; j < 18; ++j)
				features_[((i + 1) * width_ - 1) * 32 + j] *= scale;
	}

	if (height & 7) {
		const float scale = 8.0f / float(height & 7);

		for (int i = 0; i < width_ * 32; ++i)
			features_[(height_ - 1) * width_ * 32 + i] *= scale;
	}
	...
	for (int i = 0; i < height_; ++i) {
		for (int j = 0; j < width_; ++j) {
			...
			for (int k = 0; k < 9; ++k)
				sumSquare += (features_[(i * width_ + j) * 32 + k] + features_[(i * width_ + j) * 32 + k + 9]) *
	...
	for (int i = 0; i < height_; ++i) {
		for (int j = 0; j < width_; ++j) {
			...
			const float n0 = 1.0f / sqrtf(sum00 + sum01 + sum10 + sum11 + 0.0001f);
			const float n1 = 1.0f / sqrtf(sum01 + sum02 + sum11 + sum12 + 0.0001f);
			const float n2 = 1.0f / sqrtf(sum10 + sum11 + sum20 + sum21 + 0.0001f);
			const float n3 = 1.0f / sqrtf(sum11 + sum12 + sum21 + sum22 + 0.0001f);

			// Contrast-insensitive features
			for (int k = 0; k < 9; ++k) {
				const float sum = features_[(i * width_ + j) * 32 + k] + features_[(i * width_ + j) * 32 + k + 9];
				const float h0 = min(sum * n0, 0.3f);
				const float h1 = min(sum * n1, 0.3f);
				const float h2 = min(sum * n2, 0.3f);
				const float h3 = min(sum * n3, 0.3f);
				features_[(i * width_ + j) * 32 + k + 18] = (h0 + h1 + h2 + h3) / 1.2f;
			}
	...
	// The last feature is the "gradient energy" of each cell, i.e. ||C(i,j)||
	for (int i = 0; i < height_; ++i) {
		for (int j = 0; j < width_; ++j) {
			const float norm = sqrtf(features_[(i * width_ + j) * 32 + 31]);
			features_[(i * width_ + j) * 32 + 31] = min(norm / (1024 * 8 * 8 * 8), 1.0f);
		}
	}
}

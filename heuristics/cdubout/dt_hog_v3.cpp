...
void hogHeuristic::prepareForCoordinates() {
	...
	// Block normalization
	for(unsigned int i = 0; i <= nbHists_ - NB_BLOCKS; ++i) {
		for(unsigned int j = 0; j <= nbHists_ - NB_BLOCKS; ++j) {
			...
			for(unsigned int k = 0; k < NB_BLOCKS; ++k) {
				for(unsigned int l = 0; l < NB_BLOCKS; ++l) {
					for(unsigned int b = 0; b < NB_BINS; ++b) {
						m += hists_[(b * nbHists_ + i + k) * nbHists_ + j + l] *
							 hists_[(b * nbHists_ + i + k) * nbHists_ + j + l];
					}
				}
			}

			m = 1 / std::sqrt(m + 0.001);

			for(unsigned int k = 0; k < NB_BLOCKS; ++k) {
				for(unsigned int l = 0; l < NB_BLOCKS; ++l) {
					for(unsigned int b = 0; b < NB_BINS; ++b) {
						normalized_[index] =
						hists_[(b * nbHists_ + i + k) * nbHists_ + j + l] * m;
						if(normalized_[index] > 0.2) normalized_[index] = 0.2;
						n += normalized_[index] * normalized_[index];
						++index;
					}
				}
			}
			...
			for(unsigned int k = 0; k < NB_BLOCKS * NB_BLOCKS * NB_BINS; ++k) {
				normalized_[index] *= n;
				++index;
			}
		}

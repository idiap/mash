...
double MyIntegralImage::init(byte_t** pixels, unsigned int width,
	...
	// Set to zero the first row and the first column
	...
	for (unsigned int x = 1; x <= width; ++x) {
		sum += pixels[0][x - 1];
		image_[stride_ + x] = sum;
		sumSq += double(pixels[0][x - 1]) * double(pixels[0][x - 1]);
	}

	// The remaining cells are sum above and to the left
	for (unsigned int y = 2; y <= height; ++y) {
		sum = 0;

		for (unsigned int x = 1; x <= width; ++x) {
			sum += pixels[y - 1][x - 1];
			image_[y * stride_ + x] = sum + image_[(y - 1) * stride_ + x];
...
void ViolaJones::init() {
	...
	const unsigned int scale1 = max(roi_size / 8, 1U);
	const unsigned int scale2 = max(roi_size / 16, 1U);
	...
	for (unsigned int h = scale2; h <= roi_size; h += scale2) {
		for (unsigned int w = 2 * scale2; w <= roi_size; w += 2 * scale2) {
			for (unsigned int y = 0; y <= roi_size - h; y += scale1) {
				for (unsigned int x = 0; x <= roi_size - w; x += scale1) {
					rectangle.x0 = x;
					rectangle.y0 = y;
					rectangle.w0 = w;
					rectangle.h0 = h;
	...
	for (unsigned int h = 2 * scale2; h <= roi_size; h += 2 * scale2) {
		for (unsigned int w = scale2; w <= roi_size; w += scale2) {
			for (unsigned int y = 0; y <= roi_size - h; y += scale1) {
				for (unsigned int x = 0; x <= roi_size - w; x += scale1) {
					rectangle.x0 = x;
					rectangle.y0 = y;
	...
	for (unsigned int h = scale2; h <= roi_size; h += scale2) {
		for (unsigned int w = 3 * scale2; w <= roi_size; w += 3 * scale2) {
			for (unsigned int y = 0; y <= roi_size - h; y += scale1) {
				for (unsigned int x = 0; x <= roi_size - w; x += scale1) {
					rectangle.x0 = x;
					rectangle.y0 = y;
	...
	for (unsigned int h = 3 * scale2; h <= roi_size; h += 3 * scale2) {
		for (unsigned int w = scale2; w <= roi_size; w += scale2) {
			for (unsigned int y = 0; y <= roi_size - h; y += scale1) {
				for (unsigned int x = 0; x <= roi_size - w; x += scale1) {
					rectangle.x0 = x;

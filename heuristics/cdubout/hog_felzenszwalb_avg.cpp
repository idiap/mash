...
private:
	...
	double sumRed_, sumGreen_, sumBlue_;
};
...
unsigned int HoGHeuristic::dim()
{
	...
	return ((roi_size + 7) >> 3) * ((roi_size + 7) >> 3) * (32 - 18) + 9;
}
...
void HoGHeuristic::prepareForImage()
{
	...
	if (ATAN2_TABLE[0][0] == 0.0f) {
		for (int dy = -255; dy <= 255; ++dy) {
			for (int dx = -255; dx <= 255; ++dx) {
				...
				angle = max(angle * (9.0 / M_PI) + 9.0, 0.0);
	...
	sumRed_ = sumGreen_ = sumBlue_ = 0.0;
	...
	for (int y = 0; y < height; ++y) {
		for (int x = 0; x < width; ++x) {
			...
			sumRed_ += pixels[y][x].r / 255.0;
			sumGreen_ += pixels[y][x].g / 255.0;
			sumBlue_ += pixels[y][x].b / 255.0;
	...
	sumRed_ /= width * height;
	sumGreen_ /= width * height;
	sumBlue_ /= width * height;
	...
	for (int i = 0; i < height_; ++i) {
		for (int j = 0; j < width_; ++j) {
			const float sum00 = ((i > 0) && (j > 0)) ? features_[((i - 1) * width_ + j - 1) * 32 + 31] : 0.0f;
			const float sum01 = (i > 0) ? features_[((i - 1) * width_ + j) * 32 + 31] : 0.0f;
			const float sum02 = ((i > 0) && (j < width_ - 1)) ? features_[((i - 1) * width_ + j + 1) * 32 + 31] : 0.0f;
			const float sum10 = (j > 0) ? features_[(i * width_ + j - 1) * 32 + 31] : 0.0f;
			const float sum11 = features_[(i * width_ + j) * 32 + 31];
			const float sum12 = (j < width_ - 1) ? features_[(i * width_ + j + 1) * 32 + 31] : 0.0f;
			const float sum20 = ((i < height_ - 1) && (j > 0)) ? features_[((i + 1) * width_ + j - 1) * 32 + 31] : 0.0f;
			const float sum21 = (i < height_ - 1) ? features_[((i + 1) * width_ + j) * 32 + 31] : 0.0f;
			const float sum22 = ((i < height_ - 1) && (j < width_ - 1)) ? features_[((i + 1) * width_ + j + 1) * 32 + 31] : 0.0f;

			// Normalization factors
			const float n0 = 1.0f / sqrtf(sum00 + sum01 + sum10 + sum11 + 1e-12f);
			const float n1 = 1.0f / sqrtf(sum01 + sum02 + sum11 + sum12 + 1e-12f);
			const float n2 = 1.0f / sqrtf(sum10 + sum11 + sum20 + sum21 + 1e-12f);
			const float n3 = 1.0f / sqrtf(sum11 + sum12 + sum21 + sum22 + 1e-12f);
...
scalar_t HoGHeuristic::computeFeature(unsigned int feature_index)
{
	if (feature_index < 9) {
		switch (feature_index) {
			case 0: return sumRed_;
			case 1: return sumGreen_;
			case 2: return sumBlue_;
			case 3: return sumRed_ - sumGreen_;
			case 4: return sumGreen_ - sumBlue_;
			case 5: return sumBlue_ - sumRed_;
			case 6: return sumRed_ - sumGreen_ - sumBlue_;
			case 7: return sumGreen_ - sumRed_ - sumBlue_;
			case 8: return sumBlue_ - sumRed_ - sumGreen_;
		}
	}
	else {
		feature_index -= 9;
	...
	return features_[((y + i) * width_ + (x + j)) * 32 + 18 + bin];
}

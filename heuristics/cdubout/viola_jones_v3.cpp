...
void MyIntegralImage::init(byte_t** pixels, unsigned int width,
						   unsigned int height) {
...
unsigned int ViolaJones::dim() {
	return rectangles_.size() * 2;
}

void ViolaJones::prepareForImage() {
	integralImage_.init(image->grayLines(), image->width(), image->height());
}
...
scalar_t ViolaJones::computeFeature(unsigned int feature_index) {
	...
	const Rectangle& r = rectangles_[feature_index >> 1];
	...
	if (!(feature_index & 1)) {
		...
		const double norm = double(integralImage_(x + r.x0, y + r.y0, r.w0, r.h0)) * 255.0;

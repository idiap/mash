		...
		for (unsigned int cw = 1; cw <= roi_size / 3; cw <<= 1) {
			...
			for (unsigned int cy = 0; cy <= roi_size - 3 * ch; cy += (ch + 3) >> 2) {
				for (unsigned int cx = 0; cx <= roi_size - 3 * cw; cx += (cw + 3) >> 2) {
					...
					const unsigned int s00 = s[0][0] - s[0][1] - s[1][0] + s[1][1];
					const unsigned int s01 = s[0][1] - s[0][2] - s[1][1] + s[1][2];
					const unsigned int s02 = s[0][2] - s[0][3] - s[1][2] + s[1][3];
					const unsigned int s10 = s[1][0] - s[1][1] - s[2][0] + s[2][1];
					const unsigned int s11 = s[1][1] - s[1][2] - s[2][1] + s[2][2];
					const unsigned int s12 = s[1][2] - s[1][3] - s[2][2] + s[2][3];
					const unsigned int s20 = s[2][0] - s[2][1] - s[3][0] + s[3][1];
					const unsigned int s21 = s[2][1] - s[2][2] - s[3][1] + s[3][2];
					const unsigned int s22 = s[2][2] - s[2][3] - s[3][2] + s[3][3];

					const unsigned int code = ((s00 > avg)     ) |
											  ((s01 > avg) << 1) |
											  ((s02 > avg) << 2) |
											  ((s10 > avg) << 3) |
											  ((s12 > avg) << 4) |
					...
					const double dist = (std::abs(s00 - avg) +
										 std::abs(s01 - avg) +
										 std::abs(s02 - avg) +
										 std::abs(s10 - avg) +
										 std::abs(s12 - avg) +
					...
					hists_[1][code] += (1.0 - a) * (1.0 - b) * dist;
					hists_[2][code] += a * (1.0 - b) * dist;
					hists_[3][code] += (1.0 - a) * b * dist;
					hists_[4][code] += a * b * dist;
					...
					hists_[ 5 + v * 4 + u][code] += (1.0 - d) * (1.0 - e) * dist;
					hists_[ 6 + v * 4 + u][code] += d * (1.0 - e) * dist;
					hists_[ 9 + v * 4 + u][code] += (1.0 - d) * e * dist;
					hists_[10 + v * 4 + u][code] += d * e * dist;
				}
			}
		}
	}

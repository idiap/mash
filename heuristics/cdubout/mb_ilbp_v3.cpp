...
struct Intersector {
	...
	: x_(x), y_(y), w_(w), h_(h), norm_(1.0 / (w * h)) {}

	double operator()(unsigned int x, unsigned int y, unsigned int w) const {
		...
		for (unsigned int cw = 1; cw <= roi_size / 3; cw <<= 1) {
			...
			for (unsigned int cy = 0; cy <= roi_size - 3 * ch; cy += (ch + 3) >> 2) {
				for (unsigned int cx = 0; cx <= roi_size - 3 * cw; cx += (cw + 3) >> 2) {
					...
					hists_[1][code] += intersector(0, 0, half);
					hists_[2][code] += intersector(half, 0, half);
					hists_[3][code] += intersector(0, half, half);
					hists_[4][code] += intersector(half, half, half);

					// Level 2 of the pyramid
					const unsigned int quarterDown = roi_extent >> 1;
					const unsigned int quarterUp = (roi_extent + 1) >> 1;
					hists_[ 5][code] += intersector(0, 0, quarterUp);
					hists_[ 6][code] += intersector(quarterUp + 1, 0, quarterDown + 1);
					hists_[ 7][code] += intersector(half, 0, quarterDown + 1);
					hists_[ 8][code] += intersector(half + quarterDown + 1, 0, quarterUp);
					hists_[ 9][code] += intersector(0, quarterUp + 1, quarterUp);
					hists_[10][code] += intersector(quarterUp + 1, quarterUp + 1, quarterDown + 1);
					hists_[11][code] += intersector(half, quarterUp + 1, quarterDown + 1);
					hists_[12][code] += intersector(half + quarterDown + 1, quarterUp + 1, quarterUp);
					hists_[13][code] += intersector(0, half, quarterUp);
					hists_[14][code] += intersector(quarterUp + 1, half, quarterDown + 1);
					hists_[15][code] += intersector(half, half, quarterDown + 1);
					hists_[16][code] += intersector(half + quarterDown + 1, half, quarterUp);
					hists_[17][code] += intersector(0, half + quarterDown + 1, quarterUp);
					hists_[18][code] += intersector(quarterUp + 1, half + quarterDown + 1, quarterDown + 1);
					hists_[19][code] += intersector(half, half + quarterDown + 1, quarterDown + 1);
					hists_[20][code] += intersector(half + quarterDown + 1, half + quarterDown + 1, quarterUp);
				}
			}
		}
	}

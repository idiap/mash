...
private:
	...
	// The MCT histograms
	std::vector<unsigned int> hist_[3];
...
void MB_MCT::prepareForCoordinates() {
	...
	for (int c = 0; c < 3; ++c) {
		std::fill(hist_[c].begin(), hist_[c].end(), 0U);
	}
			...
			for (unsigned int cy = 0; cy <= roi_size - 3 * ch; cy += (ch + 1) >> 1) {
				for (unsigned int cx = 0; cx <= roi_size - 3 * cw; cx += (cw + 1) >> 1) {
					...
					for (int c = 0; c < 3; ++c) {
						...
						++hist_[c][(((s[0][0] - s[0][1] - s[1][0] + s[1][1]) > avg)     ) |
...
scalar_t MB_MCT::computeFeature(unsigned int feature_index) {
	return hist_[feature_index / 512][feature_index % 512];
}

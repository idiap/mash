	...
	for(int i = 0; i < 12; ++i) {
		for(int y = 1; y < height; ++y) {
			for(int x = 1; x < width; ++x) {
				sats_[i][y * width + x] += sats_[i][y * width + x - 1] +
										   sats_[i][(y - 1) * width + x] -
										   sats_[i][(y - 1) * width + x - 1];
			}
		}
	}
}
...
scalar_t HoGHeuristic::computeFeature(unsigned int feature_index) {
	...
	const int bin = rects_[feature_index].bin;

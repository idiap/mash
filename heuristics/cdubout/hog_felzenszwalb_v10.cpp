...
unsigned int HoGHeuristic::dim()
{
	...
	return ((roi_size + 7) >> 3) * ((roi_size + 7) >> 3) * 39;
}
...
void HoGHeuristic::prepareForImage()
{
	...
	// Resize the features
	features_.resize(width_ * height_ * 39);
	...
	for (int y = 0; y < height; ++y) {
		for (int x = 0; x < width; ++x) {
			...
			if (magnitude > 0.0f) {
				...
				theta = max(theta * 9.0f / float(M_PI) + 9.0f, 0.0f);
	...
	for (int i = 0; i < height_; ++i) {
		for (int j = 0; j < width_; ++j) {
			...
			for (int k = 0; k < 9; ++k)
				sumSquare += features_[(i * width_ + j) * 39 + k] * features_[(i * width_ + j) * 39 + k];

			features_[(i * width_ + j) * 39 + 31] = sumSquare;
		}
	}
	...
	for (int i = 0; i < height_; ++i) {
		for (int j = 0; j < width_; ++j) {
			float sum00 = features_[(max(i - 1, 0) * width_ + max(j - 1, 0)) * 39 + 31];
			float sum01 = features_[(max(i - 1, 0) * width_ + j) * 39 + 31];
			float sum02 = features_[(max(i - 1, 0) * width_ + min(j + 1, width_ - 1)) * 39 + 31];
			float sum10 = features_[(i * width_ + max(j - 1, 0)) * 39 + 31];
			float sum11 = features_[(i * width_ + j) * 39 + 31];
			float sum12 = features_[(i * width_ + min(j + 1, width_ - 1)) * 39 + 31];
			float sum20 = features_[(min(i + 1, height_ - 1) * width_ + max(j - 1, 0)) * 39 + 31];
			float sum21 = features_[(min(i + 1, height_ - 1) * width_ + j) * 39 + 31];
			float sum22 = features_[(min(i + 1, height_ - 1) * width_ + min(j + 1, width_ - 1)) * 39 + 31];
			...
			for (int k = 0; k < 4; ++k) {
				...
				for (int l = 0; l < 9; ++l)
					sum += min(features_[(i * width_ + j) * 39 + l] / tmp[k], 0.2f);

				features_[(i * width_ + j) * 39 + 27 + k] = sum;
			}
			...
			for (int k = 0; k < 27; ++k) {
				...
				for (int l = 0; l < 4; ++l)
					sum += min(features_[(i * width_ + j) * 39 + k] / tmp[l], 0.2f);

				features_[(i * width_ + j) * 39 + k] = sum;
			}

			const float S = tmp[0] + tmp[1] + tmp[2] + tmp[3];
			features_[(i * width_ + j) * 39 + 38] = S;
			features_[(i * width_ + j) * 39 + 32] = fabsf(tmp[0] - tmp[1]) / S;
			features_[(i * width_ + j) * 39 + 33] = fabsf(tmp[2] - tmp[3]) / S;
			features_[(i * width_ + j) * 39 + 34] = fabsf(tmp[0] - tmp[2]) / S;
			features_[(i * width_ + j) * 39 + 35] = fabsf(tmp[1] - tmp[3]) / S;
			features_[(i * width_ + j) * 39 + 36] = fabsf(tmp[0] - tmp[3]) / S;
			features_[(i * width_ + j) * 39 + 37] = fabsf(tmp[1] - tmp[2]) / S;
		}
	}
}
...
scalar_t HoGHeuristic::computeFeature(unsigned int feature_index)
{
	...
	return features_[((y + i) * width_ + (x + j)) * 39 + bin];
}
...
inline void HoGHeuristic::interpolate(int x, int y, int bin, float magnitude)
{
	...
	features_[(max(i, 0) * width_ + max(j, 0)) * 39 + bin] += magnitude * (7 - k) * (7 - l);
	features_[(max(i, 0) * width_ + min(j + 1, width_ - 1)) * 39 + bin] += magnitude * (7 - k) * l;
	features_[(min(i + 1, height_ - 1) * width_ + max(j, 0)) * 39 + bin] += magnitude * k * (7 - l);
	features_[(min(i + 1, height_ - 1) * width_ + min(j + 1, width_ - 1)) * 39 + bin] += magnitude * k * l;
}
